<template>
    <div @dragenter.prevent="toggle_active()" @dragleave.prevent="toggle_active()" @dragover.prevent @drop.prevent="drop" :class="{ 'bg-green-100 border-green-300 dark:bg-green-700': dragActive }" class="flex items-center justify-center w-full px-12 py-8 border-4 border-gray-300 border-dashed rounded dark:border-2">
        <div class="flex flex-col items-center justify-center gap-2 text-gray-500">
            <span>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-20 h-20 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- File input -->
            <label for="file" class="p-2 text-xs md:text-sm font-semibold leading-tight text-gray-600 bg-red-700 border rounded cursor-pointer hover:bg-red-800 hover:shadow-sm">
                <span class="text-red-500 dark:text-gray-100">Select a file from your device</span>
