<template>
  <div class="flex flex-col">
    <ul class="flex flex-row m-5 lg:text-2xl justify-end">
      <label for="addShow-modal" class="btn modal-button">Añadir funcion</label>
    </ul>
        ...
        <table class="table w-full">
          ...
          <tbody>
            <tr v-for="show in showsRef" :key="show.id">
              ...
              <td>
                <div class="font-bold">{{ show.show_hour }}</div>
              </td>
              <td>
                <div class="font-bold">{{ show.movie }}</div>
              </td>
              <td>
                <div class="font-bold">{{ show.room }}</div>
              </td>
...
const { shows, refreshShows, setShowActive } = useShowsStore();
let showsRef = ref(shows);

const refresh = async (force) => {
  showsRef.value = await refreshShows(force);
};
